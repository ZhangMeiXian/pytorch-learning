...
def make_batch():
    ...
    while positive != batch_size / 2 or negative != batch_size / 2:
        ...
        # MASK LM
        # 随机选择15%的token进行mask，其中80%的token采用'[MASK]'进行替换，10%的token采用随机token进行随机替换
        ...
        # Zero Paddings
        # 采用0进行padding，使得每个batch的长度相同
        n_pad = maxlen - len(input_ids)
        input_ids.extend([0] * n_pad)
        segment_ids.extend([0] * n_pad)

        # Zero Padding (100% - 15%) tokens
        # 将masked_tokens和masked_pos进行padding，使得每个batch的长度相同
        if max_pred > n_pred:
        ...
        if tokens_a_index + 1 == tokens_b_index and positive < batch_size / 2:
            # 当两个句子是同一个句子时，则为IsNext
            batch.append([input_ids, segment_ids, masked_tokens, masked_pos, True])  # IsNext
            positive += 1
        elif tokens_a_index + 1 != tokens_b_index and negative < batch_size / 2:
            # 当两个句子不是同一个句子时，则为NotNext
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        attn = nn.Softmax(dim=-1)(scores.to(device))
...
if __name__ == '__main__':
    ...
    for epoch in range(100):
        ...
        # MLM任务的损失计算
        loss_lm = criterion(logits_lm.transpose(1, 2), masked_tokens.to(device))  # for masked LM
