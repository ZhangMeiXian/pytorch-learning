...
class BiLSTM_Attention(nn.Module):
    ...
    def forward(self, X):
        ...
        hidden_state = torch.zeros(1 * 2, len(X),
                                   n_hidden).to(device)  # [num_layers(=1) * num_directions(=2), batch_size, n_hidden]
...
if __name__ == '__main__':
    ...
    for epoch in range(5000):
        optimizer.zero_grad()
        output, attention = model(inputs.to(device))
    ...
    ax.set_xticklabels([''] + ['first_word', 'second_word', 'third_word'], fontdict={'fontsize': 14}, rotation=45)
    ax.set_yticklabels([''] + ['batch_1', 'batch_2', 'batch_3', 'batch_4', 'batch_5', 'batch_6'],
                       fontdict={'fontsize': 14})
    plt.tight_layout()
    plt.savefig('Bi-LSTM_attention.png')