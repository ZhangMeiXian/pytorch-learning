# !/usr/bin/env python3
"""
Transformer with Greedy_decoder：机器翻译实战
...
if __name__ == '__main__':
    ...
    # Test
    # 贪婪解码：贪婪解码是一种简单的解码策略，在每一步选择分数最高的词作为下一个词。在这个例子中，beam
    # search的K值为1，意味着每一步都选择当前最优的路径，不考虑其他可能性。
    # 递归生成目标序列：通过逐步生成目标序列，在每一步使用模型的输出作为下一步的输入，直到生成完所有词。
    # 这个函数的设计使得在不提供完整目标序列的情况下，可以根据模型的预测逐步生成目标序列，非常适合推理和测试时使用。
